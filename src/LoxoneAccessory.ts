...
export class LoxoneAccessory {
  ...
  private configurePlannedServices(plan: AccessoryPlan): void {
    this.applyServicePlans(plan.services, plan.serviceLabels);
  }

  /**
   * Builds (or reuses) the given service plans on this accessory and prunes
   * anything left behind. Safe to call after setup for items whose services
   * only become known once a Loxone state arrives (e.g. LightControllerV2 moods).
   */
